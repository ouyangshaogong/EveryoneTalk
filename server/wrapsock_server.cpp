...
Getpeername(int fd, struct sockaddr *sa, socklen_t *salenptr)
{
	if (getpeername(fd, sa, salenptr) < 0)
		syslog(LOG_ERR, "getpeername error");
}
...
Getsockname(int fd, struct sockaddr *sa, socklen_t *salenptr)
{
	if (getsockname(fd, sa, salenptr) < 0)
		syslog(LOG_ERR, "getsockname error");
}
...
Inet6_rth_space(int type, int segments)
{
	...
	if (ret < 0)
		syslog(LOG_ERR, "inet6_rth_space error");
...
Inet6_rth_init(void *rthbuf, socklen_t rthlen, int type, int segments)
{
	...
	if (ret == NULL)
		syslog(LOG_ERR, "inet6_rth_init error");
...
Inet6_rth_add(void *rthbuf, const struct in6_addr *addr)
{
	if (inet6_rth_add(rthbuf, addr) < 0)
		syslog(LOG_ERR, "inet6_rth_add error");
}
...
Inet6_rth_reverse(const void *in, void *out)
{
	if (inet6_rth_reverse(in, out) < 0)
		syslog(LOG_ERR, "inet6_rth_reverse error");
}
...
Inet6_rth_segments(const void *rthbuf)
{
	...
	if (ret < 0)
		syslog(LOG_ERR, "inet6_rth_segments error");
...
Inet6_rth_getaddr(const void *rthbuf, int idx)
{
	...
	if (ret == NULL)
		syslog(LOG_ERR, "inet6_rth_getaddr error");
...
Sockatmark(int fd)
{
	...
	if ( (n = sockatmark(fd)) < 0)
		syslog(LOG_ERR, "sockatmark error");
	return(n);
}
...
Socketpair(int family, int type, int protocol, int *fd)
{
	...
	if ( (n = socketpair(family, type, protocol, fd)) < 0)
		syslog(LOG_ERR, "socketpair error");
}

const char *
Inet_ntop(int family, struct sockaddr_in *p_sin, char *c_ip, char *c_port, size_t len)
{
	const char	*ptr;

	if (c_ip == NULL)		/* check for old code */
		syslog(LOG_ERR, "NULL 3rd argument to inet_ntop");
	if ( (ptr = inet_ntop(family, (const void*)&p_sin->sin_addr, c_ip, len)) == NULL)
		syslog(LOG_ERR, "inet_ntop error");		/* sets errno */
    if (ntohs(p_sin->sin_port) != 0)
    {
        char c_port[SIZE];
        sprintf(c_port, ":%d", ntohs(p_sin->sin_port));
    }
...
Inet_pton(int family, const char *strptr, void *addrptr)
{
	...
	if ( (n = inet_pton(family, strptr, addrptr)) < 0)
		syslog(LOG_ERR, "inet_pton error for %s", strptr);	/* errno set */
	else if (n == 0)
		syslog(LOG_ERR, "inet_pton error for %s", strptr);	/* errno not set */
